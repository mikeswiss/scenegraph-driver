...
class SGDriver {
    ...
    launch(appId = 'dev') {
        const options = {
            method: 'POST',
            uri: `http://${this.host}:8060/launch/${appId}`
        };
    ...
    _getElementsByText(document, value) {
        const childNodes = Object.values(document.childNodes);
        const result = childNodes.reduce((acc, element) => {
            if (typeof element === 'object' && element.hasAttribute('text') && element.getAttribute('text') === value) {
                acc.push(element);
            }
            return acc;
        }, []);
    ...
    async findElements(locator) {
        ...
        switch(locator.using) {
            case 'id':
            case 'xpath':
                return this._getElementsByXpath(document, locator.value);
            case 'text':
                return this._getElementsByText(document, locator.value)
            case 'name':
